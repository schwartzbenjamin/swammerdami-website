...
Copy an existing HTML page, update its `<title>` and content, then add a new link to the `<nav>` element on each main page. Keep `styles.css` linked so the new page uses the shared design.
